...
CREATE TABLE IF NOT EXISTS `healthcare-project-487014.silver_dataset.transactions` (
...
SELECT DISTINCT 
    ...
    END AS is_quarantined
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `healthcare-project-487014.bronze_dataset.transactions_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `healthcare-project-487014.bronze_dataset.transactions_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `healthcare-project-487014.silver_dataset.transactions` AS target
USING `healthcare-project-487014.silver_dataset.quality_checks` AS source
...
CREATE TABLE IF NOT EXISTS `healthcare-project-487014.silver_dataset.encounters` (
...
CREATE OR REPLACE TABLE `healthcare-project-487014.silver_dataset.quality_checks_encounters` AS
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `healthcare-project-487014.bronze_dataset.encounters_ha`
    ...
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosb' AS datasource
    FROM `healthcare-project-487014.bronze_dataset.encounters_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `healthcare-project-487014.silver_dataset.encounters` AS target
USING `healthcare-project-487014.silver_dataset.quality_checks_encounters` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `healthcare-project-487014.silver_dataset.quality_checks_encounters`;

-------------------------------------------------------------------------------------------------------

-- 1. Create the Claims Table in BigQuery
CREATE TABLE IF NOT EXISTS `healthcare-project-487014.silver_dataset.claims` (
    Claim_Key STRING,
    SRC_ClaimID STRING,
...
CREATE OR REPLACE TABLE `healthcare-project-487014.silver_dataset.quality_checks_claims` AS
SELECT 
...
FROM (
    SELECT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `healthcare-project-487014.bronze_dataset.claims`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `healthcare-project-487014.silver_dataset.claims` AS target
USING `healthcare-project-487014.silver_dataset.quality_checks_claims` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `healthcare-project-487014.silver_dataset.quality_checks_claims`;

-------------------------------------------------------------------------------------------------------

-- 1. Create the CP Codes Silver Table in BigQuery
CREATE TABLE IF NOT EXISTS `healthcare-project-487014.silver_dataset.cpt_codes` (
    CP_Code_Key STRING,
...
CREATE OR REPLACE TABLE `healthcare-project-487014.silver_dataset.quality_checks_cpt_codes` AS
SELECT 
...
FROM (
    SELECT 
        ...
        code_status,
        'hosa' AS datasource
    FROM `healthcare-project-487014.bronze_dataset.cpt_codes`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `healthcare-project-487014.silver_dataset.cpt_codes` AS target
USING `healthcare-project-487014.silver_dataset.quality_checks_cpt_codes` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `healthcare-project-487014.silver_dataset.quality_checks_cpt_codes`;
